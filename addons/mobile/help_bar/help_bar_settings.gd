...
var _props := {
	Mobile.HelpBar.MetaProperties.TIP: {
		"updating": false, "node": "AvailableSettings/HBox/tip",
		"value": "text",
	},
	Mobile.HelpBar.MetaProperties.KEYBOARD: {
		"updating": false, "node": "AvailableSettings/HBox2/keyboard",
		"value": "selected",
	},
	Mobile.HelpBar.MetaProperties.ANCHOR: {
		"updating": false, "node": "AvailableSettings/HBox3/anchor",
		"value": "selected",
	},
...
func _ready() -> void:
	...
	%FastSettingsContainer.add_child(_fast_settings)
	...
	match node.get_class():
		"LineEdit":
			if _props[setting]["value"] == "text":
				if _update_text:
					# Prevent "text_changed" signal emission
					node.text = value
			else:
...
func get_data() -> Dictionary:
	var data := {}
	var help_bar_settings: Array = Mobile.HelpBar.MetaProperties.values()
