...
func _init() -> void:
	var _help_bar_settings: Array[Dictionary] = Mobile.HelpBar.ProjSettings.CONFIG.duplicate()
	
	for setting in _help_bar_settings:
		if not Mobile.HelpBar.FAST_SETTINGS.has(setting["path"]):
			continue
