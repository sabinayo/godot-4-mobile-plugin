...
const HELP_BAR_SETTINGS: PackedStringArray = [
...
func _start_animation() -> void:
	if not custom_enter_anim.is_empty():
	...
	if enter_anim == Mobile.HelpBar.Anims.NONE:
		return
...
func _load_settings() -> void:
	for setting in HELP_BAR_SETTINGS:
...
func delete() -> void:
	...
	if exit_anim == Mobile.HelpBar.Anims.NONE:
		queue_free()
...
func _play_sound(stream: String) -> void:
	if not stream.is_absolute_path():
		return
...
func set_settings_overrides(settings: Dictionary) -> void:
	settings_overrides = settings


## You need to implement a function with the same name and argument type.
## in your custom scene. The type of scene does not matter.
func set_edition(keyboard: int, tip: String, text: String) -> void:
	%tip.text = tip
	%tip.visible = not tip.is_empty()
