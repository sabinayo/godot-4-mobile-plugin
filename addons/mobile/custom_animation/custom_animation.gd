...
## Play a stream and automatically delete the stream player when finished.
func _play_sound(
	stream_path: String, audio_bus := "Master", volume_db: float = 0.0,
	pitch_scale: float = 1.0, mix_target := AudioStreamPlayer.MIX_TARGET_STEREO
) -> void:
	if not stream_path.is_absolute_path():
		return
...
## Play one of the stream defined in ProjectSettings and
## automatically delete the stream player when finished.
func _play_sound_from_project_settings(
	...
	match fall_back:
		_AnimationFallBacks.HELP_BAR:
			cur_anim = _AnimationFallBacks.TOOLTIP
		
		_AnimationFallBacks.TOOLTIP:
