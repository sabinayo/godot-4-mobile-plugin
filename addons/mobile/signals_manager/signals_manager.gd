...
func _on_renamed() -> void:
	if name != Mobile.SIGNALS_MANAGER_NAME:
		name = Mobile.SIGNALS_MANAGER_NAME
...
func _on_tooltip_delay_ended(
	...
	if not tooltip or not tooltip.has_method("set_mobile_tooltip"):
		push_error("Attempt to display tooltip with a custom scene which has not\
...
func _on_tooltip_safe_deletion_timer_timeout(tooltip_requester: String) -> void:
	if not tooltip_requesters.has(tooltip_requester):
		return
