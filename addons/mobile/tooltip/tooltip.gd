...
const TOOLTIP_SETTINGS: PackedStringArray = [
...
# Relative to requester position
var relative_position: String
# Relative to requester position
var cur_relative_pos: Positions
var requester_indicator_anim: String
var display_requester_indicator: bool

var _data := {}
# Use a global tween to avoid start and delete animation conflicts
var _tween: Tween

## You have to implement a function with the same name in your custom scene.
...
func set_mobile_tooltip(data: Dictionary) -> void:
	_data = data
	...
	match cur_relative_pos:
		Positions.ABOVE, Positions.BELOW:
			...
			if diff > 0.0:
				...
				%requester_indicator.global_position.y = \
					global_position.y - %requester_indicator.size.y + indicator_margin
			
			# tooltip above requester
			elif diff < 0.0:
				# Center the indicator along the y axis.
				%requester_indicator.global_position.y = \
					(global_position.y + size.y) - %requester_indicator.size.y + indicator_margin
			
			await get_tree().process_frame
			%requester_indicator.global_position.x = \
				requester_global_center.x - (%requester_indicator.size.x / 2.0)
			...
			if %requester_indicator.global_position.x <= 0.0:
				...
				%requester_indicator.global_position.x = \
					screen.x - %requester_indicator.size.x - margin - diffx
		...
		Positions.LEFT, Positions.RIGHT:
			...
			if diff > 0:
				...
				%requester_indicator.global_position.x = \
					global_position.x - (%requester_indicator.size.x / 4.0) + indicator_margin
			
			# tooltip to the left of the requester
			elif diff < 0:
				# Center the indicator along x axis
				%requester_indicator.global_position.x = \
					global_position.x + size.x - (%requester_indicator.size.x / 2.0) + indicator_margin
			
			await get_tree().process_frame
			%requester_indicator.global_position.y = \
				requester_global_center.y - (%requester_indicator.size.y / 2.0)
...
func _load_settings() -> void:
	...
	for setting in TOOLTIP_SETTINGS:
