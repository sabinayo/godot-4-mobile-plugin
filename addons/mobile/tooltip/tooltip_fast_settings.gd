@tool
class_name MobileTooltipFastSettings
extends Resource

## Settings applied to a single tooltip.
...
func _init() -> void:
	var _tooltip_settings: Array[Dictionary] = Mobile.Tooltip.ProjSettings.CONFIG.duplicate()
	
	for setting in _tooltip_settings:
		if not Mobile.Tooltip.FAST_SETTINGS.has(setting["path"]):
			continue
		
		var path := Array(setting["path"].split("/", false))
		props.append(path[-2] + "/" + path[-1])
...
func _on_internal_resource_changed(property: StringName, value: Variant) -> void:
	property_changed.emit(Mobile.Tooltip.ProjSettings.SAVE_PATH + property, value)
...
func _get(property: StringName) -> Variant:
	...
	if has_meta(prop):
		...
		if value is Resource and not value.changed.is_connected(_on_internal_resource_changed):
			value.changed.connect(
				Callable(self, "_on_internal_resource_changed").bind(
					property, value
				)
...
func _get_property_list() -> Array[Dictionary]:
	...
	for setting in _tooltip_settings:
		if not Mobile.Tooltip.FAST_SETTINGS.has(setting["path"]): continue
...
func _property_get_revert(property: StringName) -> Variant:
	...
	return ProjectSettings.get_setting(
		Mobile.Tooltip.ProjSettings.CONFIG_PATH + prop,
		ProjectSettings.get_setting(
			Mobile.Tooltip.ProjSettings.DISPLAY_PATH + prop
		)
	)
