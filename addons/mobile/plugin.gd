...
const SAVE_FILE := DATA_DIR + "/settings.cfg"
const SAVE_FILE_SECTION := "ProjectSettings"

var _plugin_settings: Array[Dictionary]
...
func _enter_tree() -> void:
	...
	_plugin_settings = Mobile.HelpBar.ProjSettings.CONFIG.duplicate()
	_plugin_settings.append_array(Mobile.Tooltip.ProjSettings.CONFIG.duplicate())
	...
	if FileAccess.file_exists(SAVE_FILE):
		...
		var err: int = cfg.load(SAVE_FILE)
		
		if err != OK:
			return
		
		for key in cfg.get_section_keys(SAVE_FILE_SECTION):
			ProjectSettings.set_setting(
				key, cfg.get_value(SAVE_FILE_SECTION, key)
			)
...
func _exit_tree() -> void:
	...
	for setting in _plugin_settings:
